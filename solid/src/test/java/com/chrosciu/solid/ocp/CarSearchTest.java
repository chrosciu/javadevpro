...
public class CarSearchTest {
    ...
    public void shouldBeAbleToPerformSearchByGivenColors() {
        List<Car> foundCars = carSearch.findBy(new CarColorsPredicate(EnumSet.of(WHITE, BLACK)));

        assertThat(foundCars).containsExactlyInAnyOrder(FIAT_PUNTO, VW_PASSAT);
    }

    @Test
    public void shouldBeAbleToPerformSearchByMaxPriceAndGivenColors() {
        List<Car> foundCars = carSearch.findBy(
                new CarMaxPricePredicate(1200)
                .and(new CarColorsPredicate(EnumSet.of(WHITE, BLACK)))
        );
